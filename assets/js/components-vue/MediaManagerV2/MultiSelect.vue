<template>
    <div class="multi-select" :class="{ 'open-upwards': openUpwards }">
        <div class="multi-select__selected" @click="toggleDropdown">
            <span v-if="selectedItems.length === 0" class="select-placeholder">Select {{ title }}</span>
                    ...
                    >
                        ...
                        <div class="option-content">
                            <span class="option-label">
                                {{ option.label }}
                                <span v-if="badgeKey && option[badgeKey]" class="badge rounded-pill bg-secondary ms-1">
                                    {{ option[badgeKey] }}
                                </span>
                            </span>
    ...
    computed: {
        filteredOptions() {
            ...
            return this.options.filter(option => {
                ...
                const badge = this.badgeKey && option[this.badgeKey]
                    ? option[this.badgeKey].toLowerCase()
                    : '';
